def max_bars(n, bar):
    ...
    :param n: 입력되는 막대의 개수
    :param bar: 막대의 왼쪽 끝점과 오른쪽 끝점
    :return: 하나의 못으로 꽂을 수 있는 막대의 최대 개수, 즉 막대가 가장 많이 겹치는 지점의 막대 개수
    """
    # 입력된 값과 막대의 개수가 일치하는지 확인한다.
    assert len(bar) == n

    # 왼쪽 끝 점과 오른쪽 끝 점을 감소시키기 위해 각각 1과 -1로 표시한다.
    coordinate = []
    for a, b in bar:
        coordinate.append((a, 1))  # 왼쪽 끝점에 해당하면, 1을 append한다.
        coordinate.append((b, -1))  # 오른쪽 끝점에 해당하면, -1을 append한다.
    ...
    for _, value in coordinate:
        current_bars_count += value  # 각각의 coordinate 요소의 증감값을 갱신한다.
        if value == 1:  # value가 1이라면, 수직선이 새로운 막대의 왼쪽 끝점에 도달했다는 것을 의미한다.
...
bar = []
for _ in range(n):
    bar.append(list(map(int, input().split())))

print(max_bars(n, bar))
